...
type UserCollectionJSON = Record<
  string,
  { normalOwned: boolean; foilOwned: boolean; isWishlist?: boolean; price?: string; note?: string }
>;
  ...
  useEffect(() => {
    async function loadWishlistData() {
      ...
      if (data && data.collection) {
        ...
        // Requêtes parallélisées + Cache local IndexedDB
        const cardPromises = wishIDs.map(async (cardId) => {
          const cacheKey = `wishlist_card_${cardId}`;
          const cached = await get<WishlistCard>(cacheKey);
          ...
          try {
            ...
            if (res.ok) {
              ...
              const formattedCard: WishlistCard = {
                id: cardData.id,
              ...
              await set(cacheKey, formattedCard);
        ...
        setWishlistCards(resolvedCards.filter((c): c is WishlistCard => c !== null));
      } else {
        setWishlistCards([]);
      }
      setLoading(false);
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white p-4 md:p-10 relative">
      ...
      >
        ...
        <div
          className={`relative w-80 bg-slate-900 border-r border-slate-800 h-full shadow-2xl p-6 flex flex-col justify-between z-10 transition-transform duration-300 ease-out ${
        ...
        >
          <div>
            ...
            <div className="space-y-3">
              ...
              <Link
                href="/wishlist"
                className="w-full text-left bg-purple-950/40 border border-purple-800/50 p-3.5 rounded-xl font-semibold text-sm text-purple-300 transition flex items-center gap-3 cursor-pointer"
              >
          ...
          <div className="pt-6 border-t border-slate-800">
            {currentUser ? (
              ...
              <Link
                href="/compte"
                className="block text-center bg-white text-slate-900 text-xs font-bold p-3 rounded-xl hover:bg-gray-200 transition shadow-md"
              >
      ...
      <div className="max-w-6xl mx-auto pt-6 md:pt-0">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-extrabold mb-2 bg-gradient-to-r from-red-400 to-pink-500 bg-clip-text text-transparent">
            🎯 Ta Chasse aux Cartes (Wishlist)
          </h1>
          <p className="text-slate-400 text-sm">
            Toutes les pépites qu&apos;il te manque pour compléter tes extensions
          </p>
        </div>
        ...
        <div className="mb-8 flex justify-center">
          <Link
            href="/"
            className="px-6 py-2.5 rounded-full text-sm font-bold transition flex items-center gap-2 shadow-lg bg-slate-800 text-slate-300 hover:bg-slate-700 border border-slate-600"
          >
        ...
        {!currentUser ? (
          <div className="text-center bg-slate-900/50 border border-slate-800 rounded-xl p-10 mt-8">
            ...
            <p className="text-slate-300 text-base font-semibold mb-4">
              Connecte-toi pour retrouver ta liste de chasse.
            </p>
            <Link
              href="/compte"
              className="inline-block bg-white text-slate-900 text-xs font-bold px-5 py-3 rounded-xl hover:bg-gray-200 transition shadow-md"
            >
              Se connecter
            </Link>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
            {wishlistCards.map((card) => (
              <div
                key={card.id}
                className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col justify-between shadow-lg relative group"
              >
                <button
                ...
                <div>
                  <div className="mb-3 flex justify-center bg-slate-950/50 p-2 rounded-lg border border-slate-800/60 min-h-[176px] items-center relative overflow-hidden">
                    {card.image ? (
                      <Image
                        src={card.image}
                        alt={card.name}
                  ...
                  <div className="flex justify-between items-start mb-1 gap-1">
                    ...
                    <span className="text-[10px] bg-slate-800 text-slate-400 px-1.5 py-0.5 rounded-md shrink-0">
                      #{card.localId}
                    </span>
                  </div>
                ...
                <div className="space-y-1.5 pt-2 border-t border-slate-800">
                  ...
                  <div className="grid grid-cols-3 gap-1 text-[11px] font-semibold">
                    <a
                      href={`https://www.cardmarket.com/fr/Pokemon/Products/Search?searchString=${encodeURIComponent(
                        card.name
                      )}`}
                      target="_blank"
          ...
          <div className="text-center bg-slate-900/50 border border-slate-800 rounded-xl p-10 mt-8">
            ...
            <p className="text-slate-500 text-xs">
              Va dans ton Pokedex et clique sur le cœur des cartes que tu recherches activement !
            </p>
          </div>
        )}
      </div>
